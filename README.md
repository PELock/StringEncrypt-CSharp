# StringEncrypt — String & File Encryption for C# / .NET Developers

[![License](https://img.shields.io/badge/license-Apache%202.0-blue.svg)](LICENSE)
[![.NET](https://img.shields.io/badge/.NET-8.0-512BD4.svg)](https://dotnet.microsoft.com/)
[![NuGet](https://img.shields.io/badge/nuget-StringEncrypt-004880.svg)](https://www.nuget.org/packages/StringEncrypt/)
...
```csharp
string hiddenCSharpString = "How to encrypt strings in C#? That's how!";
```

## Encrypted string in C# (UNICODE) source code format
...
* You can automate the encryption process in your builds using our `WebAPI` interface (`C#` / .NET, `JavaScript`, `PHP`, and `Python` bindings)
...
* ```result.Error (int)``` - error code (see `ErrorCode` in the assembly)
* ```result.Source (string?)``` - decryptor source code
* ```result.Expired (bool?)``` - expiration flag
* ```result.CreditsLeft (int?)``` - number of credits left
* ```result.CreditsTotal (int?)``` - initial number of credits

#### Error codes:

* ```ErrorCode.Success (0)``` - everything went fine
* ```ErrorCode.EmptyLabel (1)``` - label parameter is empty
* ```ErrorCode.LengthLabel (2)``` - label length is too long
* ```ErrorCode.EmptyString (3)``` - input string is empty
* ```ErrorCode.EmptyBytes (4)``` - input file bytes array is empty
* ```ErrorCode.EmptyInput (5)``` - input source (either string or file) is missing
* ```ErrorCode.LengthString (6)``` - string length is too long
* ```ErrorCode.InvalidLang (7)``` - programming language not supported
* ```ErrorCode.InvalidLocale (8)``` - language locale is not supported
* ```ErrorCode.CmdMin (9)``` - invalid number of minimum encryption commands
* ```ErrorCode.CmdMax (10)``` - invalid number of maximum encryption commands
* ```ErrorCode.LengthBytes (11)``` - bytes/file length is too long
* ```ErrorCode.Demo (100)``` - you need a valid code to use full version features
...
* ```result.Demo (bool?)``` - demo mode flag
* ```result.LabelLimit (int?)``` - label limit length
* ```result.StringLimit (int?)``` - string/file limit length
* ```result.CreditsLeft (int?)``` - number of credits left
* ```result.CreditsTotal (int?)``` - initial number of credits
* ```result.CmdMin (int?)``` - minimum number of encryption commands
* ```result.CmdMax (int?)``` - maximum number of encryption commands
...
* Official StringEncrypt .NET package published on NuGet (`StringEncrypt`)
